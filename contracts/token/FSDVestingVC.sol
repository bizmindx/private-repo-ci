...
contract FSDVestingVC is IFSDVesting {
    /* ========== LIBRARIES ========== */
    ...
    uint256 private constant CLIFF = 12 * ONE_MONTH;
    // Duration of a vest
    uint256 private constant DURATION = 24 * ONE_MONTH;
    // Vesting duration after cliff
    uint256 private constant LINEAR_VEST_AFTER_CLIFF = 12 * ONE_MONTH;
    ...
    function calculateVestingClaim() public view returns (uint256) {
        ...
        if (block.timestamp >= start.add(DURATION)) {
            vestedAmount = amount;
        } else if (block.timestamp >= start.add(CLIFF)) {
            vestedAmount = amount.mul(5) / 100; // 5% unlock
            uint256 linearClaim = amount.sub(vestedAmount).mul(
                block.timestamp.sub(start.add(CLIFF))
            ) / LINEAR_VEST_AFTER_CLIFF;
            vestedAmount = vestedAmount.add(linearClaim);
        } else {
            return 0;
        }
        return vestedAmount.sub(totalClaimed);
    }
